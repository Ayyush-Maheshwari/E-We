document.addEventListener('DOMContentLoaded', function() {
    ...
    // Fetch car data from your Express server
    const fetchCarData = () => {
        const carContainer = document.getElementById('carContainer');
        fetch('http://localhost:5500/cardata')
            .then(response => {
                if (!response.ok) {
            ...
            .then(data => {
                data.forEach(car => {
                    ...
                    carImage.src = `./${car.images}.png`;
                    carButton.appendChild(carImage);

                    const carDetailsList = document.createElement('ul');

                    const carNo = document.createElement('li');
                    carNo.textContent = `Car No: ${car.car_no}`;
                    carDetailsList.appendChild(carNo);

                    const model = document.createElement('li');
                    model.textContent = `Model: ${car.model}`;
                    carDetailsList.appendChild(model);

                    const price = document.createElement('li');
                    price.textContent = `Price: ₹${car.price_day}/day`;
                    carDetailsList.appendChild(price);

                    const availability = document.createElement('li');
                    availability.textContent = `Availability: ${car.availability}`;
                    carDetailsList.appendChild(availability);

                    carButton.appendChild(carDetailsList);

                    carButton.addEventListener('click', function() {
                        document.querySelectorAll('.car').forEach(function(btn) {
